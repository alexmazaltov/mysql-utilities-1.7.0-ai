...
class CachingSha2PasswordAuthPlugin(BaseAuthPlugin):
    def auth_response(self):
        if not self._password:
            return b''

        hash1 = sha256(self._password.encode('utf-8')).digest()
        hash2 = sha256(hash1).digest()
        hash3 = sha256(self._auth_data + hash2).digest()

        return b'\x01' + bytes(x ^ y for x, y in zip(hash1, hash3))
...
def get_auth_plugin(plugin_name):
    ...
    if plugin_name == 'caching_sha2_password':
        return CachingSha2PasswordAuthPlugin
